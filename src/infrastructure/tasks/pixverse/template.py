...
from ...orm.database.models import PixverseTemplates

from ...orm.database.repositories import PixverseTemplateRepository


class PixverseTemplateCelery(PixverseCelery):
    def __init__(
        self,
    ) -> None:
        super().__init__()
        self._repository = PixverseTemplateRepository(
    ...
    def database_files_list(
        self,
        database_files: list[PixverseTemplates],
    ) -> list[str]:
        return [
    ...
    async def clean_files(
        ...
        database_files: list[PixverseTemplates] = await self._repository.fetch_all()
